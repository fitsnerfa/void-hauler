# VOID HAULER — Instruction Manual (v1.5.0)
...
**Starting out:** click **Launch / Continue** on the title screen. Progress is stored in the browser and protected by the Black Box Flight Recorder described in §21. Starting a new game requires confirmation and archives the current flight first.
...
|---|---|
| **W / ↑** | Thrust forward |
| **S / ↓** | Reverse thrust (half power) |
| **A / D** or **←/→** | Turn |
| **SPACE** (hold) | Fire mining laser |
| **SHIFT** or **CTRL** (hold) | Activate Tractor Beam and/or Ram Scoop *(once installed — same key does both)* |
| **Z** | Fire a missile — auto-targets the nearest threat *(once you have a launcher)* |
| **E** | Dock at a station/outpost, or activate a Stargate when nearby |
| **I** | Open the Cargo Manifest (also where you smelt ore) |
| **J** | Open the Quest Journal |
| **T** | Open material transfer when near the Contract Vessel |
...
| Material | Symbol | Base Price | Notes |
|---|---|---|---|
| Iron Ore | Fe | 5 cr | Most common |
| Copper Ore | Cu | 13 cr | Common |
| Silver Ore | Ag | 32 cr | Uncommon |
| Gold Ore | Au | 78 cr | Rare |
| Platinum Ore | Pt | 155 cr | Very rare |
| Ruby | Ru | 260 cr | Gem, very rare |
| Sapphire | Sa | 260 cr | Gem, very rare |
| Diamond | Di | 650 cr | Gem, extremely rare |
| Quantum Crystal | Qx | 420 cr | Extremely rare |
| Dark Matter | Dm | 20,000 cr | Special — see §9 |
| Scrap Metal | Sc | 90 cr | Corvette salvage only — see §17 |
...
| Upgrade | Effect | Cap |
|---|---|---|
| **Cargo Expansion** | More cargo hold space | Unlimited |
| **Mining Laser** | More laser damage | Unlimited |
| **Engine Core** | Top speed, acceleration, turn rate | Unlimited |
| **Hull Plating** | Max hull integrity | Unlimited |
| **Energy Cell** | Energy capacity & regen rate | Unlimited |
| **Tractor Beam** | Pull range & strength | Level 4 (6 with a Faction perk) |
| **Deflector Shield** | Shield capacity & regen | Level 4 (6 with a Faction perk) |
| **Mineral Scanner** | Identifies revealed asteroid composition at range | Level 2 (full screen) |
| **Gas Tank** | Separate gas storage capacity | Unlimited *(requires Ram Scoop first)* |
| **Mule Bay** | Cargo hold for your Contract Vessel | Unlimited *(requires Escort Level 50)* |

### One-time modules (milestone-gated, bought once)
| Module | Unlock Condition | What it does |
|---|---|---|
| **Missile Battery** | Salvaged from a destroyed Frigate | Lets you fire missiles with Z (see §14) |
| **Smelter** | Captain Level 6 | Refine raw ore into ingots (see §10) |
| **Ram Scoop** | 15 Frigate kills (Frigate Bounty quest) | Collect gas from clouds in the Nebula (see §16) |
...
| Role | Bonus |
|---|---|
| **Mining Officer** | +laser damage |
| **Navigator** | +ship speed & turn rate |
| **Quartermaster** | +cargo capacity |
| **Weapons Specialist** | +missile damage & faster reload |
| **Metallurgist** | +ingot sale value |
| **Gas Engineer** | +Ram Scoop collection volume |
...
| Perk | Requires | Cost | Effect |
|---|---|---|---|
| Trade License | Associate | 150 FP | 10% off all ship upgrades |
| Tractor Overcharge Clearance | Trusted Contractor | 300 FP | Raises Tractor Beam cap to 6 |
| Shield Overcharge Clearance | Trusted Contractor | 300 FP | Raises Shield cap to 6 |
| Deep Space Charts | Veteran Partner | 500 FP | Always shows Dark Matter on radar |
| Escort Contract | Elite Operative | 900 FP | Unlocks the Contract Vessel ally ship |
...
1. Destroy **5 pirates**
2. Collect **1 Dark Matter chunk**
3. Reach **Veteran Partner** Faction rank
...
| Type | Bay Cost | Damage | Notes |
|---|---|---|---|
| **Ship Missile** | 1 slot | Scales with upgrades/crew | Homes in on any hostile ship |
| **Dark Matter Missile (DMM)** | 10 slots | 20% of target's max HP | Only targets Dark Matter, hard-capped at 3 carried |
...
1. Open the Faction tab and click **Accept Contract** on the Frigate Bounty card.
2. Destroy **15 Frigates** in the Deep Zone. Kills you already had before accepting still count.
3. Once accepted, a live **X/15** tracker appears in your main HUD (below your credit total) so you don't need to check the menu to see progress.
...
| Gas | Symbol | Price | Rarity |
|---|---|---|---|
| Hydrogen | H | 4 cr | Very common |
| Helium | He | 9 cr | Common |
| Nitrogen | N | 22 cr | Uncommon |
| Methane | CH4 | 45 cr | Uncommon |
| Neon | Ne | 180 cr | Rare |
| Xenon | Xe | 500 cr | Very rare |
...
VOID HAULER stores the active game in the browser. **Export Save** downloads a read-only JSON copy and never changes the active game. **Import Save** opens File Explorer so you can select a previously exported JSON file.

### Black Box Flight Recorder

The title screen includes a **Black Box** button. It contains the current flight plus up to **15 recovery flight logs**.

- Up to **5 flight logs** may be protected with **Progression Lock**.
- Locked logs are never removed by automatic rotation.
- Attempting to lock a sixth log displays a warning and leaves the log unlocked.
- Restoring a flight log first archives the current timeline, then reloads the selected state.
- This allows an older Level 20 path and a newer Level 40 path to coexist.
- Unlocked logs rotate only after the 15-log capacity is reached.

### Save triggers

| Save type | Trigger | Creates flight log? |
|---|---|---|
| Quick save | Every 2 minutes during active play | No |
| Event save | Trading, crafting, upgrades, equipment changes, transfers, and other meaningful actions | No |
| Emergency checkpoint | First save after five minutes have passed since the last recovery snapshot | Yes |
| Captain milestone | Captain gains a level | Yes |
| Contract Vessel milestone | Contract Vessel gains a level or certification | Yes |
| Boss milestone | A named WANTED pirate is defeated | Yes |
| Contract milestone | Contract Vessel is awarded | Yes |
| Safety archive | Before importing, restoring, or starting a new game | Yes |

### Starting over

The reset option requires typing **NEW GAME**. The existing flight is archived before the active save is removed, and locked flight logs remain available.
...
## 23. Named WANTED Pirates

Named pirates are persistent boss encounters assigned to danger zones. They display a flashing **WANTED** label, a boss health bar, a pulsing radar marker, unique ship colors, and two escorts. Defeating one awards a bounty, Scrap Metal, and a key piece.

## 24. Contract Vessel Progression

The Frigate Bounty awards a Contract Vessel. It gains experience while operating with the captain. FP purchases upgrades after certification milestones.

| Vessel Level | Certification |
|---:|---|
| 25 | Tier I |
| 50 | Tier II |
| 75 | Tier III |
...
Upgradeable systems include Mining Laser, Cargo Hold, Gas Hold, Engines, and Hull Plating. Standard upgrades stop at Tier V. Visible cargo pods, gas cylinders, mining hardware, armor, and thrusters appear as systems improve. Press **T** near the vessel to transfer materials in either direction.
